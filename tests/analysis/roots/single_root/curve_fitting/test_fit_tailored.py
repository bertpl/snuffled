...
@pytest.mark.parametrize("c_true", [0.5, 1.0, 2.0])
...
def test_fit_curve_with_uncertainty_tailored(
    ...
    a_opt, b_opt, c_opt = fit_curve_tailored(x_values, fx_values, range_b, range_c, reg)
...
def test_param_step_a():
    ...
    # --- act ---------------------------------------------
    a_m, b_m, c_m = param_step(a, b, c, "a", -1.0, range_b, range_c)
...
def test_param_step_b():
    ...
    # --- act ---------------------------------------------
    a_m, b_m, c_m = param_step(a, b, c, "b", -1.0, range_b, range_c)
    a_p, b_p, c_p = param_step(a, b, c, "b", +1.0, range_b, range_c)
...
def test_param_step_c():
    ...
    # --- act ---------------------------------------------
    a_min, b_min, c_min = param_step(a, b, c, "c", -10.0, range_b, range_c)
    a_m, b_m, c_m = param_step(a, b, c, "c", -1.0, range_b, range_c)
    a_p, b_p, c_p = param_step(a, b, c, "c", +1.0, range_b, range_c)
    a_max, b_max, c_max = param_step(a, b, c, "c", +10.0, range_b, range_c)
...
def test_param_step_ac(c: float, step_size: float):
    ...
    a_new_expected, _, _ = param_step(a, b, c, "a", step_size, range_b, range_c)

    # --- act ---------------------------------------------
    a_new, b_new, c_new = param_step(a, b, c, "ac", step_size, range_b, range_c)
...
def test_param_step_ba(c: float, step_size: float):
    ...
    _, b_new_expected, _ = param_step(a, b, c, "b", step_size, range_b, range_c)

    # --- act ---------------------------------------------
    a_new, b_new, c_new = param_step(a, b, c, "ba", step_size, range_b, range_c)
    gx_after = fitting_curve(np.array([1 / r, r]), a_new, b_new, c_new)
...
def test_param_step_bc(c: float, step_size: float):
    ...
    # --- act ---------------------------------------------
    a_new, b_new, c_new = param_step(a, b, c, "bc", step_size, range_b, range_c)
    gx_after = fitting_curve(np.array([1 / r, r]), a_new, b_new, c_new)
